...
const fullHeadingRef = useTemplateRef('question-header')
const fullQuestionRef = useTemplateRef('question')
...
async function animateIn() {
  ...
  await fullHeadingRef.value?.animate()
  await fullQuestionRef.value?.animate()
    ...
    <div v-if="question" class="game-question--container text-h4 py-4 d-flex align-center justify-center text-center" style="min-height: 300px;">
      <div class="game-question">
        <div class="text-h2 pb-3 font-weight-thin">
          <TypingText ref="question-header" :text="`Frage ${props.currentQuestionNr}`" :tick="40" />
        </div>
        <TypingText ref="question" :text="props.question?.question ?? ''" :tick="15" />
      </div>
    </div>
    <v-item-group
